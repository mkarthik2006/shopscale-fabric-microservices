...
public interface InboxEventRepository extends JpaRepository<InboxEventEntity, UUID> {
    ...
    int updateStatusIfCurrent(@Param("id") UUID id,
                              @Param("expectedStatus") com.shopscale.inventory.model.InboxEventStatus expectedStatus,
                              @Param("newStatus") com.shopscale.inventory.model.InboxEventStatus newStatus);
}
